...
- включает `gdm3.service` (или `gdm.service`), делает `graphical.target`
  умолчанием;
- проверяет, что рядом с ним лежат `configure-gnome.service`,
  `configure.sh`, `get_cloud_user.sh` и `daemon.conf`, и **отказывает
  (код 1)**, если чего-то нет: без этих файлов первой загрузки не будет
  вовсе. Путь берётся от каталога самого скрипта, а не от зашитого
  `/opt/vmsetup/gnome/`, — проверка отвечает на вопрос «файл приехал рядом
  со мной?», а не «раскладка всё ещё такая?»;
- кладёт `/etc/systemd/system/configure-gnome.service` из своего каталога
  и включает его.

**`yq` расширение не ставит, и больше его не требует.** Раньше
`configure.sh` начинался с `command -v yq` и без него отказывался
настраивать рабочий стол — при том что `yq` сам не вызывал ни разу.
Подробности — в «Порядок в VMFILE».
...
- проверяет предпосылки и при нехватке любой отказывает (код 1): исполняемый
  `get_cloud_user.sh` рядом, шаблон `daemon.conf` рядом;
...
которого рабочий стол настраивается. Отказ при этом приезжал на первой
загрузке устройства — после записи носителя, на машине, у которой может
не быть ни монитора, ни сети.
...
и тогда автологин настроится на него. Запасной путь об этом говорит вслух,
в stderr:
...
- не ставит `yq` (см. выше), не проверяет его ни на сборке, ни на первой
  загрузке и сам его не вызывает;
...
- **в журнале `Note: cloud-init user unavailable; falling back to '<имя>'`**
  ...
  названная учётка не та, которую настраивает cloud-init, добавьте
  `EXTENSION yq` выше этой строки и пересоберите образ;
