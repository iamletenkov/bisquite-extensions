...
if [ -z "$CAMERA_PKG_REL" ]; then
    # Профиль платы без камер (AGX Xavier). Не предупреждение: ждать драйверы
    # там, где их не объявляли, — ложная тревога, а от ложных тревог перестают
    # читать настоящие.
    echo "профиль камер не объявляет (CAMERA_PKG_REL пуст) — пропуск"
elif [ ! -d "$CAMERA_SRC" ]; then
