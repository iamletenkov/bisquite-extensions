...
|---|---|
| `CODE_SERVER_PORT` | HTTP-порт |
| `CODE_SERVER_PASSWORD` | Пароль; `none` включает `auth: none` |
| `CODE_SERVER_USER` | Пользователь; пусто — берётся из cloud-init |
...
**`PASSWORD: none` означает `auth: none` и `bind-addr: 0.0.0.0`** — сервер
отдаёт шелл любому в сети. Установка говорит об этом предупреждением в
журнале сборки; это единственное место, где решение видно.
