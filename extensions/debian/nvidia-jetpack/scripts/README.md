...
* **С USB он не грузится вовсе** — ограничение стека, не настройка. Носитель
  системы только NVMe в слоте M.2 Key M. На запись это не влияет: шаг `08`
  переписывает `root=` на `PARTUUID`, поэтому один образ верен для обеих шин,
  и диск можно писать в USB-боксе, а потом переставлять в плату.
