...
for f in bisquite-teleport teleport.service bisquite-teleport-apps.path bisquite-teleport-apps.service \
...
install -m 0755 "$SCRIPT_DIR/bisquite-teleport" /usr/local/sbin/bisquite-teleport
...
install -d -m 0755 /etc/bisquite/teleport /etc/bisquite/teleport/apps.d
if [[ ! -f /etc/bisquite/teleport/config ]]; then
    install -m 0600 /dev/null /etc/bisquite/teleport/config
    cat > /etc/bisquite/teleport/config <<'CONF'
# Подключение к Teleport. Менять командами, они же применяют:
#   sudo bisquite-teleport join TELEPORT_PROXY=… TELEPORT_TOKEN=… TELEPORT_ENV=…
#   sudo bisquite-teleport set TELEPORT_ENV=…
TELEPORT_PROXY=
TELEPORT_TOKEN=
TELEPORT_CA_PIN=
TELEPORT_ENV=
