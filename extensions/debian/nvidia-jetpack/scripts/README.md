...
Умолчания всех скриптов — **Jetson AGX Orin Developer Kit** (модуль
p3701-0000, носитель p3737-0000) на **JetPack 6.2 / L4T 36.4.3** с загрузкой
с NVMe и окружением для GMSL2-камер Sensing `SG8A-AGON-G2Y-A1`. Другая плата
задаётся **профилем**, а не правкой скриптов, — см. следующий раздел.

## Профиль платы

Всё платозависимое — версия L4T со ссылками и суммами, XML разметки, конфиг
QSPI, пакет камер, цель `flash.sh` и SKU модуля — вынесено в переменные
окружения. Набор значений на одну плату лежит в `boards/<плата>.env`:

| Профиль | Плата | L4T |
|---|---|---|
| `boards/orin-agx.env` | AGX Orin Developer Kit (p3701-0000 + p3737-0000) | 36.4.3 (JetPack 6.2) |
| `boards/xavier-agx.env` | AGX Xavier Developer Kit (p2972-0000) | 35.6.5 (JetPack 5.1.x) — **не прогнан на железе** |
...
**Профиль AGX Orin ничего не меняет — он объявляет.** Значения в нём те же,
что зашиты умолчаниями; смысл файла в том, чтобы «что именно собирается»
читалось в одном месте, а не собиралось по десяти скриптам.
...
**Шагу `09` для последнего шага нужен не только `bs`, но и `virt-inspector`.**
`bs image import` проверяет образ на наличие ОС и сверяет архитектуру, а
делает это `virt-inspector` из `libguestfs-tools`. Расширение его не ставит
намеренно: станция — машина прошивки, bisquite на ней ставит тот, кому нужна
регистрация, и вместе с bisquite приходят его зависимости. Если `bs` стоит
из чекаута с venv, системных зависимостей рядом нет:

```bash
sudo apt-get install -y libguestfs-tools
```

Без него шаги 0-6 отработают полностью, а шаг 7 упадёт — образ при этом
уже собран и лежит на диске (проверено на станции 2026-09-18).
...
Руками их по одной задавать не нужно: согласованный набор на плату — это
`boards/<плата>.env`, см. раздел «Профиль платы» выше.
