...
# Пусто = «у этой платы QSPI нет» (AGX Xavier: загрузчик в eMMC модуля).
# Раскрытие через `-`, иначе профиль, обнуливший цель, получил бы оринскую.
...
if [ "$TARGET" = qspi ] && [ -z "$QSPI_TARGET" ]; then
    echo "ОТКАЗ: профиль не объявляет конфигурации QSPI."
    echo "У этой платы отдельного QSPI нет — загрузчик лежит во внутренней"
    echo "eMMC модуля и пишется вместе с ней: --target emmc."
    exit 1
fi
